...
public class RenderEventBase {
	public static RenderEventBase Instance;
	...
	public void pre(RenderPlayerEvent.Pre event){
		...
		RenderMPMBase.Instance.setModelData(data, player);
		...
		if(!data.resourceInit && lastSkinTick > MaxSkinTick){
			RenderMPMBase.Instance.loadResource((AbstractClientPlayer) player);
			lastSkinTick = 0;
...
//		}
		...
		RenderMPMBase.Instance.setEntity(entity);
	...
	public void special(RenderPlayerEvent.Specials.Pre event){
		...
		if(event.renderCape){
			if(!data.cloakInnit && RenderEventBase.lastCapeTick > RenderEventBase.MaxSkinTick){
				// data.cloakObject = renderer.loadCapeResource((AbstractClientPlayer) event.entityPlayer);
				RenderEventBase.lastCapeTick = 0;
