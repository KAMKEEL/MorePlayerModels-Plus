...
public class RenderMPMBase extends RenderPlayerBase {
	...
	public void afterPassSpecialRender(EntityLivingBase base, double x, double y, double z)
	{
		...
		if (this.canRenderSee(base))
			ChatMessages.getChatMessages(player.getCommandSenderName()).renderMessages(x, y + 0.7 + player.height, z);
	}

	@Override
	public void afterDoRenderShadowAndFire(Entity player, double p_76986_2_, double p_76986_4_, double p_76986_6_, float p_76986_8_, float p_76986_9_){
		super.afterDoRenderShadowAndFire(player, p_76986_2_, p_76986_4_, p_76986_6_, p_76986_8_, p_76986_9_);
	}

	protected boolean canRenderSee(EntityLivingBase p_110813_1_)
	{
	...
	public void renderItem(EntityPlayer par1AbstractClientPlayer) {
		...
		if (itemstack1 != null)
		{
			...
			this.renderPlayer.modelBipedMain.bipedRightArm.postRender(0.0625F);
			...
			if (itemstack1.getItem().requiresMultipleRenderPasses())
			{
				for (k = 0; k <= itemstack1.getItem().getRenderPasses(itemstack1.getItemDamage()); ++k)
				{
					...
					RenderManager.instance.itemRenderer.renderItem(par1AbstractClientPlayer, itemstack1, k);
				}
			}
			else
			{
				...
				RenderManager.instance.itemRenderer.renderItem(par1AbstractClientPlayer, itemstack1, 0);
			}
	...
	@Override
	public void rotatePlayer(AbstractClientPlayer par1EntityLiving, float par2, float par3, float par4)
	{
        if(!((EntityPlayer) par1EntityLiving).isEntityAlive()){
			super.rotatePlayer(par1EntityLiving, par2, par3, par4);
			return;
		}

		if(((EntityPlayer) par1EntityLiving).ridingEntity != null){
		...
		if (data.animation == EnumAnimation.SLEEPING_EAST){
			...
			GL11.glRotatef(getDeathMaxRotation((EntityPlayer) par1EntityLiving), 0.0F, 0.0F, 1.0F);
	...
	public void renderHelmet(EntityPlayer entityPlayer) {
		...
		this.renderPlayer.modelBipedMain.bipedHead.postRender(0.0625F);
		...
		if (itemstack.getItem() instanceof ItemBlock)
		{
			...
			RenderManager.instance.itemRenderer.renderItem(entityPlayer, itemstack, 0);
		}
	...
	public void renderBackitem(EntityPlayer player) {
		...
		renderPlayer.modelBipedMain.bipedBody.postRender(0.065f);
